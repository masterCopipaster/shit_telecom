...
static signed char sym0_curve[MOD_SAMLES_PER_BIT];
static signed char sym1_curve[MOD_SAMLES_PER_BIT];

#define CONFIG_EXAMPLE_DAC_WRITE_ASYNC 0
...
{
    ESP_LOGI(TAG, "Audio size %d bytes, played at frequency %d Hz asynchronously", data_size, MOD_SAMPLING_FREQ);
    uint32_t cnt = 1;
    while (1) {
...
{
    ESP_LOGI(TAG, "Audio size %d bytes, played at frequency %d Hz synchronously", data_size, MOD_SAMPLING_FREQ);
    uint32_t cnt = 1;
    while (1) {
        printf("Play count: %"PRIu32"\n", cnt++);
